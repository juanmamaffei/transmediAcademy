class TestsController < ApplicationController
  ...
  def show
    ...
    redirect_to root_path, notice: 'No tenés permiso para esto, chinwenwencha.' unless @permissions >= 10
  end
    ...
    if @permissions >= 20
      ...
      respond_to do |format|
        if @test.update(test_params)
          format.html do
            redirect_to content_test_path(@content, @test),
                        notice: 'Modificaste una prueba. Ya no se van a machetear...'
          end
  ...
  def corregir
    ...
    if @test.correct == ans
      ...
      redirect_to content_path(@test.content_id),
                  notice: 'Te equivocaste... No pasa nada, pero quizás necesites volver a revisar el contenido.'
    end
  end
